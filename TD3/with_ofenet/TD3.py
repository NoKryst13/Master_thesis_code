...
class TD3(object):
	...
	def train(self, replay_buffer, batch_size=256):
		...
		with torch.no_grad():
			...
			next_states_features = self.ofenet.features_from_states(next_state)
			...
			next_action = (
				self.actor_target(next_states_features) + noise
			).clamp(-self.max_action, self.max_action)

			next_sa_features = self.ofenet.features_from_states_actions(next_state, next_action)
		...
		sa_features = self.ofenet.features_from_states_actions(state, action)
		...
		if self.total_it % self.policy_freq == 0:
			states_features = self.ofenet.features_from_states(state)
			policy_actions = self.actor(states_features)
			sa_features = self.ofenet.features_from_states_actions(state, policy_actions)
			# Compute actor losse
