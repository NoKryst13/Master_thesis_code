import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import math
from progress.bar import Bar
from torch import distributions as pyd
import sys

from agent import Agent
import utils

import hydra
import copy
import gym
import sys
from network import OFENet
from misc import get_target_dim


class SACAgent(Agent):
    """SAC algorithm."""
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
                 actor_cfg, discount, init_temperature, alpha_lr, alpha_betas,
                 actor_lr, actor_betas, actor_update_frequency, critic_lr,
                 critic_betas, critic_tau, critic_target_update_frequency,
                 batch_size, learnable_temperature, aux_task, env_name, total_units):
        super().__init__()

        if env_name == "Humanoid-v2":
            self.reward_scale = 20
            self.max_action = 0.4
        else:
            self.reward_scale = 5
            self.max_action = 1.0

        if aux_task == "fsp" or aux_task == "fsdp":
            dim_target = get_target_dim(env_name)

        elif aux_task == "rwp":
            dim_target = 1

        if env_name == "HalfCheetah-v2" or env_name == "Humanoid-v2":
            num_layers = 8
        else:
            num_layers = 6

        self.ofenet = OFENet(
            dim_state=obs_dim,
            dim_action=action_dim,
            dim_output=dim_target,
            total_units=total_units,
            num_layers=num_layers,
            aux_task=aux_task,
            env_name=env_name,
            skip_action_branch=False
        )

        self.action_range = action_range
        self.device = torch.device(device)
        self.discount = discount
        self.critic_tau = critic_tau
        self.actor_update_frequency = actor_update_frequency
        self.critic_target_update_frequency = critic_target_update_frequency
        self.batch_size = batch_size
        self.learnable_temperature = learnable_temperature

        self.critic = hydra.utils.instantiate(critic_cfg).to(self.device)
        self.critic_target = hydra.utils.instantiate(critic_cfg).to(
            self.device)
        self.critic_target.load_state_dict(self.critic.state_dict())

        self.actor = hydra.utils.instantiate(actor_cfg).to(self.device)

        self.log_alpha = torch.tensor(np.log(init_temperature)).to(self.device)
        self.log_alpha.requires_grad = True
        # set target entropy to -|A|
        self.target_entropy = -action_dim

        # optimizers
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(),
                                                lr=actor_lr,
                                                betas=actor_betas)

        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(),
                                                 lr=critic_lr,
                                                 betas=critic_betas)

        self.log_alpha_optimizer = torch.optim.Adam([self.log_alpha],
                                                    lr=alpha_lr,
                                                    betas=alpha_betas)

        self.train()
        self.critic_target.train()

    def train(self, training=True):
        self.training = training
        self.actor.train(training)
        self.critic.train(training)

    @property
    def alpha(self):
        return self.log_alpha.exp()

    def act(self, obs, sample=False):
        obs = torch.FloatTensor(obs).to(self.device)
        obs = obs.unsqueeze(0)
        obs = self.ofenet.features_from_states(obs)
        dist = self.actor(obs)
        action = dist.sample() if sample else dist.mean
        action = action * self.max_action
        action = action.clamp(*self.action_range)
        assert action.ndim == 2 and action.shape[0] == 1
        return utils.to_np(action[0])

    def update_critic(self, obs, action, reward, next_obs, not_done, step):
        next_s_features = self.ofenet.features_from_states(next_obs)

        dist = self.actor(next_s_features)
        next_action = dist.rsample()
        log_prob = dist.log_prob(next_action).sum(-1, keepdim=True)
        next_action = next_action * self.max_action

        next_sa_features = self.ofenet.features_from_states_actions(next_obs, next_action)

        target_Q1, target_Q2 = self.critic_target(next_sa_features)
        target_V = torch.min(target_Q1,
                             target_Q2) - self.alpha.detach() * log_prob
        target_Q = reward + (not_done * self.discount * target_V)
        target_Q = target_Q.detach()

        # get current Q estimates
        sa_features = self.ofenet.features_from_states_actions(obs, action)
        current_Q1, current_Q2 = self.critic(sa_features)
        critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(
            current_Q2, target_Q)

        # Optimize the critic
        self.critic_optimizer.zero_grad(set_to_none=True)
        critic_loss.backward()
        self.critic_optimizer.step()

    def update_actor_and_alpha(self, obs, step):
        s_features = self.ofenet.features_from_states(obs)

        dist = self.actor(s_features)
        action = dist.rsample()
        log_prob = dist.log_prob(action).sum(-1, keepdim=True)
        action = action * self.max_action

        sa_features = self.ofenet.features_from_states_actions(obs, action)

        actor_Q1, actor_Q2 = self.critic(sa_features)

        actor_Q = torch.min(actor_Q1, actor_Q2)
        actor_loss = (self.alpha.detach() * log_prob - actor_Q).mean()

        # optimize the actor
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()

        if self.learnable_temperature:
            self.log_alpha_optimizer.zero_grad()
            alpha_loss = (self.alpha *
                          (-log_prob - self.target_entropy).detach()).mean()
            alpha_loss.backward()
            self.log_alpha_optimizer.step()

    def update(self, replay_buffer, step):
        obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample(
            self.batch_size)

        self.update_critic(obs, action, reward, next_obs, not_done_no_max,
                          step)

        if step % self.actor_update_frequency == 0:
            self.update_actor_and_alpha(obs, step)

        if step % self.critic_target_update_frequency == 0:
            utils.soft_update_params(self.critic, self.critic_target,
                                     self.critic_tau)

    def pretrain_ofe(self, env, replay_buffer, random_collect, batch_size):
        episode_return = 0
        episode_timesteps = 0
        state = env.reset()
        self.ofenet.state_model.train()
        self.ofenet.action_model.train()
        with Bar("Collecting Pretrain Samples:", max=random_collect) as bar:
            for i in range(random_collect):
                action = env.action_space.sample()
                next_state, reward, done, _ = env.step(action)
                episode_return += reward
                done_flag = done
                if episode_timesteps == env._max_episode_steps:
                    done_flag = False
                replay_buffer.add(obs=state, action=action, next_obs=next_state, reward=reward, done=done_flag,
                                  done_no_max=done_flag)
                state = next_state
                if done:
                    state = env.reset()
                    episode_timesteps = 0
                    episode_return = 0
                bar.next()

        with Bar("Pretraining:", max=random_collect) as bar:
            for i in range(random_collect):
                sample_states, sample_actions, sample_rewards, sample_next_states, sample_dones, sample_dones_ = replay_buffer.sample(
                    batch_size=batch_size)
                self.ofenet.train_ofe(sample_states, sample_actions, sample_next_states, sample_rewards,
                                      sample_dones)
                bar.next()
        self.ofenet.state_model.eval()
        self.ofenet.action_model.eval()

def get_default_scale_reward(env_name):
    if env_name.startswith("Ant"):
        return 5.0
    elif env_name.startswith("Walker2d"):
        return 5.0
    elif env_name.startswith("HalfCheetah"):
        return 5.0
    elif env_name.startswith("Hopper"):
        return 5.0
    elif env_name.startswith("Humanoid"):
        return 20.0
    else:
        raise ValueError("received an invalid environment name.")